  ...
  methods: {
    async getListPokemon() {
      ...
      const details = await Promise.all(
        data.results.map((r) =>
          fetch("https://pokeapi.co/api/v2/pokemon/" + r.name).then((res) =>
            res.json(),
          ),
        ),
      );
...
<template>
  <div
    class="min-h-screen bg-gradient-to-b from-[#DC0A2D] via-[#B50721] to-[#8B0000] flex flex-col items-center justify-center px-4 py-8"
  >
    <!-- Decorative top bar -->
    <div
      class="absolute top-0 left-0 right-0 h-3 bg-[#1a1a1a] flex items-center justify-center gap-2 py-1 animate-fade-down"
    >
      <div
        class="dot-led dot-yellow w-2 h-2 rounded-full bg-yellow-400 shadow-[0_0_6px_#FECA1B]"
      ></div>
    ...
    <div
      class="pokedex-card relative w-full max-w-xl rounded-3xl border-4 border-[#2C2C2C] bg-[#1a1a1a] shadow-2xl shadow-black/50 overflow-hidden mt-6"
    >
      <!-- Inner screen border -->
      <div
        class="rounded-2xl border-4 border-[#4a4a4a] m-2 bg-gradient-to-b from-[#2d2d2d] to-[#1a1a1a] p-8"
      >
        <!-- Logo / Title -->
        <div class="text-center mb-10 animate-fade-in">
          <h1
            class="text-3xl md:text-4xl font-black tracking-widest text-white drop-shadow-lg poke-title poke-title-glow"
          >
            POKÉKITA
          </h1>
          <p
            class="text-yellow-400 text-sm mt-2 tracking-widest font-semibold animate-fade-in"
        ...
        <div class="flex flex-col items-center gap-4">
          <div
            class="search-wrap w-full max-w-md bg-[#0d0d0d] rounded-2xl p-2 border-2 border-[#4a4a4a] shadow-inner transition-all duration-300 focus-within:border-yellow-500/60 focus-within:ring-2 focus-within:ring-yellow-500/20"
          >
            <input
              v-model="search"
              type="text"
          ...
          <p class="text-gray-500 text-xs">
            Contoh: Pikachu, Charmander, Charizard
          </p>
          <div class="flex flex-wrap items-center gap-x-1 gap-y-0 text-sm">
            ...
            >
              <span
                class="text-white hover:text-yellow-500 cursor-pointer transition-colors duration-200"
                @click="viewedDetailPokemon(suggestion.name)"
                >{{ suggestion.name }}</span
              >
              <span
                v-if="index < suggestionList.length - 1"
                class="text-gray-500"
                >|</span
              >
            </template>
    ...
    <div class="w-full max-w-4xl mt-8 px-4">
      <Transition name="view-switch" mode="out-in">
        <div
          v-if="!viewedAsDetail"
          key="list"
          class="rounded-2xl border-4 border-[#2C2C2C] bg-[#1a1a1a] shadow-2xl shadow-black/50 overflow-hidden"
        >
          <div
            class="rounded-xl border-2 border-[#4a4a4a] m-2 bg-[#0d0d0d] p-4 min-h-[200px] relative"
          >
            <div
              v-if="loading"
              class="absolute inset-0 bg-[#0d0d0d]/80 z-10 flex justify-center items-center rounded-xl transition-opacity duration-300"
              aria-hidden="true"
            >
            ...
            <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-3">
              ...
              >
                <img
                  ...
                  :alt="pokemon.name"
                  class="w-18 h-18 mx-auto object-contain transition-transform duration-300 group-hover:scale-110"
                />
                <p class="text-white font-semibold mt-2 capitalize">
                  {{ pokemon.name }}
                </p>
            ...
            <div class="mt-2 flex justify-center gap-1 flex-wrap items-center">
              <template
                ...
                :key="item.type === 'ellipsis' ? `e-${index}` : item.num"
              >
                <span v-if="item.type === 'ellipsis'" class="text-gray-500 px-1"
                  >...</span
                >
                <button
                  v-else
        ...
        >
          <div
            class="rounded-xl border-2 border-[#4a4a4a] m-2 bg-[#0d0d0d] p-6"
            v-if="selectedPokemon"
          >
            <div class="flex justify-end">
              <button
                @click="viewedAsDetail = false"
                class="mb-4 bg-yellow-500 hover:bg-yellow-400 hover:scale-105 active:scale-95 text-[#1a1a1a] font-bold px-4 py-2 rounded-lg transition-all duration-200"
              >
                ← View All Pokemon
              </button>
            </div>
            ...
            <div class="flex flex-col md:flex-row gap-6">
              ...
              <div class="flex-shrink-0 text-center md:w-1/3 animate-detail-in">
                ...
                />
                <p class="text-white text-2xl font-bold mt-3 capitalize">
              ...
              <div class="flex-1 space-y-4 animate-detail-in animate-delay-1">
                <div class="grid grid-cols-2 gap-4">
                  <div
                    class="bg-[#2d2d2d] rounded-xl p-3 border border-[#4a4a4a] transition-all duration-300 hover:border-yellow-500/30"
                  >
                ...
                <div
                  class="bg-[#2d2d2d] rounded-xl p-4 border border-[#4a4a4a] transition-all duration-300 hover:border-yellow-500/30"
                >
                  ...
                  <div class="space-y-2">
                    ...
                    >
                      <span class="text-gray-400 capitalize text-sm w-24">{{
                        s.stat.name.replace("-", " ")
                      }}</span>
                      <div
                        class="flex-1 h-2 bg-[#1a1a1a] rounded-full overflow-hidden"
                      >
                        <div
                ...
                <div
                  class="bg-[#2d2d2d] rounded-xl p-4 border border-[#4a4a4a]"
                  v-if="selectedPokemon.abilities?.length"
                >
    ...
    <!-- Footer hint -->
    <a
      href="https://www.instagram.com/alifakbar_8/"
      class="mt-8 text-white/90 text-sm animate-fade-in"
      target="_blank"
      >@alifakbar_8</a
    >
  </div>
